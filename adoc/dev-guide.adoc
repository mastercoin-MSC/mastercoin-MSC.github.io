...
Mastercoin Team
v0.1.3, September 29, 2014: Early draft
:numbered:
:toc:
:toclevels: 3
:linkattrs:
...
http://github.com/mastercoin-MSC/mastercore[Master Core] is the new reference implementation for Mastercoin and is available in experimental form.

Maste Core is based upon https://github.com/bitcoin/bitcoin[Bitcoin Core] -- the reference implementation of Bitcoin. Mastercore adds support for the Master Protocol, while leaving the existing Bitcoin functionality unchanged.
...
The Mastercore implementation of Mastercoin includes an http://www.jsonrpc.org[JSON-RPC] API that allows access to key Mastercoin features over the HTTP(S) protocol. As Mastercore is based upon Bitcoin Core, the Mastercoin JSON-RPC API will be a superset of the API provided by Bitcoin.
...
The https://github.com/mastercoin-MSC/mastercore/blob/mscore-0.0.8/doc/apidocumentation.md[Master Core RPC API Documentation] is available on Github.
